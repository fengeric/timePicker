package com.umeng.test.photoSelector.view;
...
import com.umeng.test.photoSelector.util.Util;

/**
 * 2013-5-28 自定义自动换行LinearLayout
 */
public class FixGridLayout extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		try {
			...
			Util.showLog(getClass(),
					"onLayout(boolean changed, int l, int t, int r, int b)", e);
		}
	}

	private int getNum(int count) {// 获取图片行数，每行3个图片
		try {
			count = (count - 1) / 3 + 1;
		} catch (Exception e) {
			Util.showLog(getClass(), "getNum(int count)", e);
		}
		return count;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(
				resolveSize(mCellWidth * getNum(count), widthMeasureSpec),
				resolveSize(mCellHeight * getNum(count), heightMeasureSpec));
